...
lifting1$Sex <- as.factor(lifting1$Sex)
lifting1 %>%
  ggplot(aes(x = factor(Sex), y = TotalKg, group = Sex)) +
  geom_boxplot() +
  facet_wrap(~ AgeClass) +
  labs(x = "Body weight (kg)", y = "Total Weight Lifting (kg)") +
...
$$[ TotalWeightLifting | bodyweight ] = \beta_0 + \beta_1 bodyweight + \beta_2 sexM + \beta_3 ageAgeClass20-23 + \beta_4 AgeClass24-34 + \beta_5 AgeClass35-39 + \beta_5 AgeClass40-44$$
```{r}
