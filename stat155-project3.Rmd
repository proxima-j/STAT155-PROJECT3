...
# set.seed(10000)
# my_sample <- sampel_n(lifting, size = 2000, replace = FALSE)
...
lifting1 %>%
  ...
  geom_line(size = 0.5) +
  geom_smooth(aes(group = AgeClass), 
...
lifting1 %>%
  ggplot(aes(x = BodyweightKg, y = TotalKg, group = Age, color = Sex)) + 
  geom_line(size = 0.5) +
  geom_smooth(se = FALSE, size = 1.5, color = "black", method = "loess") +
  facet_wrap(~ Sex) +
...
$$[ TotalWeightLifting | bodyweight ] = \beta_0 + \beta_1 bodyweight + \beta_2 sex + \beta_3 age$$
```{r}
mod1 <- lm(with(TotalKg ~ BodyweightKg + Sex + Age, data = lifting1))
coef(mod1)
glance(mod1)

# mod2 <- lm(with(TotalKg ~ BodyweightKg + Sex + Age+BodyweightKg:Sex, data = lifting1))
# coef(mod2)
# glance(mod2)
```
